<?php 
  ...
  $detail_slide=executeQuery($sql,false);
  if(isset($_POST['submit'])){
    ...
    $imageFileType = pathinfo($filename,PATHINFO_EXTENSION);
      ...
      elseif($filename==''){
        $update_slide = "UPDATE slider SET description='$desc',
          url = '$link' WHERE id = $id";
            executeQuery($update_slide);
        ...
        if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"){
          ...
          $update_slide = "UPDATE slider SET description='$desc', image = 'public/images/$filename',
          url = '$link' WHERE id = $id";
            executeQuery($update_slide);
          move_uploaded_file($tmp_name,$url.$filename);
          header("location:".Base_url."admin/slider");
    ...
    <section class="content container-fluid">
      <form style="margin-bottom: 20px;font-family: time new roman"  action="" method="post" enctype="multipart/form-data" class="container-fluid">
          <div class="rows col-lg-6 form-group">
            ...
            <br>
            <label>Link</label>
            <input class="col-3 form-control container-fluid form-control-default" type="text" name="link" id="exampleFormControlFile1" value="<?php echo $detail_slide['url'] ?>">
            <span class="text-danger"><?php if(isset($errLink)) echo $errLink; ?></span>
            <br>
            <img style="height: 200px;" class="img-thumbnail col-lg-12 form-group" width="385px" alt="Responsive image" src="<?php echo Base_url.$detail_slide['image'] ?>">
            ...
            <textarea id="editor1" name="mota" rows="10" cols="1000">
                <?php echo $detail_slide['description'] ?>
            </textarea>
